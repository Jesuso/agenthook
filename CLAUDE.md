...
- `src/store.js` — JSON files in `dataDir`: `secrets.json` (handshake secrets keyed by webhook
  ...
  recovery), `attempts.json` (per-`(ref,step)` run counts backing the `changes`-loop cap), and
...
The normalized unit passed engine-wide is the **job**: `{ kind: 'pipeline', ref, stepId, dedupKey, comment? }`.
